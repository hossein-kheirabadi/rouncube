...
//        return strtotime($date);
        return $this->jdate('Y/n/j ,H:i:s',strtotime($date));
    ...
    public function jstrftime($format,$timestamp='',$none='',$time_zone='Asia/Tehran',$tr_num='fa'){
        ...
        for($i=0; $i<$sl; $i++){
            ...
            switch($sub){
                ...
                case'c':
                    $key=$his->jdate_words(array('rh'=>$date[6],'mm'=>$j_m));
    ...
    public function jmktime($h='',$m='',$s='',$jm='',$jd='',$jy='',$is_dst=-1){
        $h=$this->tr_num($h); $m=$this->tr_num($m); $s=$this->tr_num($s);
    ...
    public function jalali_to_gregorian($j_y,$j_m,$j_d,$mod=''){
        $g_y=$this->tr_num($j_y); $j_m=$this->tr_num($j_m); $j_d=$this->tr_num($j_d);/* <= :اين سطر ، جزء تابع اصلي نيست */
        $d_4=($j_y+1)%4;
